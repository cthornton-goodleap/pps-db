...
BEGIN
	...
	FOR rec IN SELECT * FROM public.payment_party_configurations LOOP
		...
		VALUES
			(rec."tenantId", 'MIGRATION', 'MIGRATION', utcNow, utcNow, false, false, new_party_id, (select id from party.party_identifier_types where name = 'DirectoryId' limit 1), 'MIGRATION');
    END LOOP;
